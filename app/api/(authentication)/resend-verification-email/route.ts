...
function getVerificationToken(user: IUser): string {
  ...
  const numberOfRetries = user?.numberOfRetries || 0;
  user.numberOfRetries = numberOfRetries + 1;
...
export const POST = async (request: Request) => {
  try {
    ...
    if (user.numberOfRetries >= 5) {
      return new NextResponse(
        JSON.stringify({
          message:
            "You have maxed out the attemps to verify your email. Please write us an email at awayme.cc and we will get back to you!",
        }),
        { status: 400 }
      );
    }
